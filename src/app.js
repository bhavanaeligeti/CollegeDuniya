...
const lawModel = require('./models/addlaw');
const checklaw = require('./models/lawstudent');
const dcollegeModel = require('./models/dcollege')
const mbaUser = require("./models/mbastudent");
const aiimsModel = require("./models/addaiims");
const lcollegeModel = require('./models/lcollege')
...
function dcheckEmail(req, res, next) {
  var email = req.body.email;
  var checkexitemail = dcollegeModel.findOne({ email: email });
  checkexitemail.exec((err, data) => {
    if (err) throw err;
    if (data) {
      return res.render('mbbscollege', {title:'', msg:'Email Already Exist' });
    }
    next();
  });
}

function lcheckEmail(req, res, next) {
  var email = req.body.email;
  var checkexitemail = lcollegeModel.findOne({ email: email });
  checkexitemail.exec((err, data) => {
    if (err) throw err;
    if (data) {
      return res.render('lawcollege', {title:'', msg:'Email Already Exist' });
    }
    next();
  });
}
...
app.post('/forget', async (req,res)=> {
  ...
  if(err) {
    ...
    res.render("forget",{title:"college dunia", msg:"Gmail or Mobile Number Wrong",succ:'',errors:'',gmail:''})
  }
  })
})
  ...
  checkUser.exec((err, data) => {
    if (data == null) {
      ...
      if (password === getPassword) {
        ...
        // var getUserID = data._id;
        console.log(getPassword)
        if(username == "real@gmail.com" && password == "123456"){
          res.redirect('/admin')
        }
        else {
...
app.post("/llogin", (req, res) => {
  var username = req.body.username;
  const password = req.body.psw;
  const checkUser = lcollegeModel.findOne({ email: username });
  checkUser.exec((err, data) => {
    if (data == null) {
...
app.post("/mlogin", (req, res) => {
  var username = req.body.username;
  const password = req.body.psw;
  const checkUser = mcollegeModel.findOne({ email: username });
  checkUser.exec((err, data) => {
    if (data == null) {
...
app.get('/lawsignup', function (req, res, next) {
  res.render('lawsignup', { title: 'College Dunia', msg: '' });
});
...
app.get('/lawcollege', function (req, res, next) {
  res.render('lawcollege', { title: 'College Dunia', msg: '' });
});
...
app.get('/mbbscollege', function (req, res, next) {
  res.render('mbbscollege', { title: 'College Dunia', msg: '' });
});
...
app.post('/lawsignup', function (req, res, next) {
...
app.post('/mbbscollege',dcheckEmail, function (req, res, next) {
  var fname = req.body.fname;
  var lname = req.body.lname;
  var mnumber = req.body.mnumber;
  var date = req.body.date;
  ...
  const stuid = 1;
    ...
    var userDetails = new dcollegeModel({
      fname:fname,
      lname:lname,
      password:password,
      date:date,
...
app.post('/lawcollege',lcheckEmail, function (req, res, next) {
  var fname = req.body.fname;
  var lname = req.body.lname;
  var mnumber = req.body.mnumber;
  var date = req.body.date;
  ...
  const stuid = 1;
    ...
    var userDetails = new lcollegeModel({
      fname:fname,
      lname:lname,
      password:password,
      date:date,
...
app.get('/addlaw', (req, res) => {
  res.render('addlaw',{msg:''})
})

app.post('/addlaw',(req,res)=>{
  ...
  var state = req.body.state;
    ...
    userDetails.save((err, doc) => {
      if (err) throw err;
      res.render('addlaw', { title: 'College', msg: 'College added successfully!' });
    });
})
...
app.get('/aiims',async (req, res) => {

  const stdata = mbaUser.find({})
  const getstu = aiimsModel.find({})
  try {
    let getdata = await getstu.exec(); 
    let stdatas = await stdata.exec();
    res.render('aiims',{data:getdata,stdatas:stdatas})
  }
  catch(error) {
    throw error;
  }
...
app.get('/laws',async (req, res) => {
  const data = lawModel.find({})
  try {
    let record = await data.exec();
    res.render('laws',{records:record})
  }
  catch(err){
   throw Error;
  }
...
app.get('/law',async (req, res) => {
  ...
  const stdata = checklaw.find({})
  const getstu = lawModel.find({})

  try {
    let getdata = await getstu.exec(); 
    let stdatas = await stdata.exec();
    res.render('law',{data:getdata,stdatas:stdatas})
  }
  catch(error) {
    throw error;
  }
...
app.get('/student', async (req,res)=> {
  var loginUser = localStorage.getItem('loginUser');
  if(loginUser) {
  const datas = updateModel.find({})
  const gets = feesModel.find({})
  const datae = updateModel.find({})
  const userdata = bcollegeModel.findOne({email:loginUser})
  try {
    let fdata = await gets.exec();
    let udata = await userdata.exec();
    let updata = await datae.exec()
    let data = await datas.exec()
    res.render('student',{mdata:fdata,udata:udata,updata:updata,data:data})
  }
  catch(error) {
    throw error;
  }
...
app.get('/mbastudent', async (req,res)=> {
  var loginUser = localStorage.getItem('loginUser');
  if(loginUser) {
  const datas = updateModel.find({})
  const gets = feesModel.find({})
  const datae = updateModel.find({})
  const userdata = mcollegeModel.findOne({email:loginUser})
  try {
    let fdata = await gets.exec();
    let udata = await userdata.exec();
    let updata = await datae.exec()
    let data = await datas.exec()
    res.render('mbastudent',{mdata:fdata,udata:udata,updata:updata,data:data})
  }
  catch(error) {
    throw error;
  }
...
app.get('/mbbsstudent', async (req,res)=> {
  var loginUser = localStorage.getItem('loginUser');
  if(loginUser) {
  const datas = updateModel.find({})
  const gets = feesModel.find({})
  const datae = updateModel.find({})
  const userdata = checkmbbsModel.findOne({email:loginUser})
  try {
    let fdata = await gets.exec();
    let udata = await userdata.exec();
    let updata = await datae.exec()
    let data = await datas.exec()
    res.render('mbbsstudent',{mdata:fdata,udata:udata,updata:updata,data:data})
  }
  catch(error) {
    throw error;
  }
...
app.get('/lawstudent', async (req,res)=> {
  var loginUser = localStorage.getItem('loginUser');
  if(loginUser) {
  const datas = updateModel.find({})
  const gets = feesModel.find({})
  const datae = updateModel.find({})
  const userdata = lcollegeModel.findOne({email:loginUser})
  try {
    let fdata = await gets.exec();
    let udata = await userdata.exec();
    let updata = await datae.exec()
    let data = await datas.exec()
    res.render('lawstudent',{mdata:fdata,udata:udata,updata:updata,data:data})
  }
  catch(error) {
    throw error;
  }
...
app.get('/update',async function(req, res, next) {
  ...
  try {
    let record = await udata.exec()
    res.render('update', { title: 'College Dunia', errors: '',success:"",records:record });
  }
  catch(err){
    throw err;
  }
